...
* 3.1 旧版本SDK无对应字段，后台默认发送广播

* 3.2 新版本SDK默认不发广播，由业务控制是否需要发送广播
...
| options?   | {enableNotification?: boolean} | 是否需要发送广播开关 |
...
示例：
```js
    room.deleteUserAttributesByKeys({ keys, options }).then(res => {
...
示例：
```js
    room.clearUserAttributes({options}).then(res => {
...
示例：
```js
    room.deleteroomAttributesByKeys({ keys, options }).then(res => {
...
示例：
```js
    room.clearRoomAttributes({options}).then(res => {
